...
server <- function(input, output, session){
  ...
  # the timer for the score answer
  ...
  # check for score answer and update score
  observe({
  ...
  output$game_results <- renderUI({
    if(end_game() && score()>0){
      HTML(glue("<b style='color:red'>Game Over!<br>Your Final Score is {score()} !</b>"))
    }
    else {
      HTML(glue("
              <b>
